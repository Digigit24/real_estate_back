...
def _ensure_uuid_tenant_id(tenant_id, context):
    """Validate tenant_id format and return normalized string form."""
    ...
    if not tenant_id_str:
        return None
    try:
        return str(uuid.UUID(tenant_id_str))
    except (ValueError, TypeError):
        logger.warning(f"Invalid tenant_id in {context}: {tenant_id}")
        raise ValidationError({
            'tenant_id': 'Invalid tenant ID format. Expected UUID.'
        })
...
class TenantMixin(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Ensure tenant_id is not empty string and is a valid UUID
        tenant_id = _ensure_uuid_tenant_id(
            tenant_id,
            f"serializer create for {request.method} {request.path}"
        )
        if tenant_id is None:
            logger.error(
...
class TenantViewSetMixin:
    ...
    def get_queryset(self):
        ...
        tenant_id = _ensure_uuid_tenant_id(
            tenant_id,
            f"get_queryset for {self.request.method} {self.request.path}"
        )
        if tenant_id is None:
            logger.warning(
    ...
    def perform_create(self, serializer):
        ...
        # Ensure tenant_id is not empty string and is UUID
        tenant_id = _ensure_uuid_tenant_id(
            tenant_id,
            f"perform_create for {self.request.method} {self.request.path}"
        )
        if tenant_id is None:
            logger.error(
